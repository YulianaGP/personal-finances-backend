...
// ✅ Middleware para actualización completa de deuda (PUT)
export default function validateDebtFullUpdate(req, res, next) {
  const {
    debt_name,
    creditor_name,
    amount,
  ...
  // --- 1. Todos los campos obligatorios deben venir en PUT ---
  const requiredError = validateRequiredFields(req.body, [
    "debt_name",
    "creditor_name",
    "amount",
    "due_date",
    "contributors", // en PUT debe venir siempre
  ]);
  if (requiredError) {
